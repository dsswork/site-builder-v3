<x-admin.layout.layout title="Edit Template">
    <form action="{{ route('admin.templates.update', compact('template')) }}"
        ...
        <x-cards.simple title="Cover" half>
            <img src="{{ asset('storage/' . $template->cover) }}" class="w-100" alt="">
        </x-cards.simple>
    ...
    @if(!$template->template)
    <form action="{{ route('admin.templates.add-level', compact('template')) }}"
          method="post"
          class="d-inline"
    >
        @csrf
...
<script>
    ...
    function removeCard(cardId) {
        setTimeout(() => document.querySelector('#' + cardId).remove(), 500);
    }
</script>
